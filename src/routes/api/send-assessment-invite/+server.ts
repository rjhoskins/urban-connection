...
// 	console.log('GET /api/send-assessment-invite==================> ');
// 	return json({ message: 'GET /api/send-assessment-invite', 'application/json': '' });
// };

export const POST: RequestHandler = async (event) => {
	const res = await event.request.json();
	console.log('POST /api/send-assessment-invite============> ', res);
	try {
		if (dev) {
...
async function sendProdEmail({
	...
	try {
		...
		const mailOptions = {
			...
			subject: 'The Urban Connection Project | Take The Assessment! | ' + subject,
		...
		console.log('Prod asessment email sent: ' + result.response);
	} catch (error) {
		console.error('Error sending Prod assessment email: ', error);
	}
}
...
async function sendDevEmail({
	...
	try {
		...
		const info = await transporter.sendMail({
			...
			text: 'Hello world?',
			html: `<a href="${inviteLink}">Hello ✔ Assessment Invite Link ${text}</a>` //just link
		});
